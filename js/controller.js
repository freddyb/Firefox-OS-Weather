...
(function() {
  App.controller('MainController', [
    '$rootScope', '$timeout', 'Options', 'City', function($rootScope, $timeout, Options, City) {
      var cities, options;
      ...
      options = Options.get();
      cities = City.getAll().map(function(a) {
        return a.name;
      });
      if (options.start !== Options.defaultOptions.start) {
        if (cities.indexOf(options.start) === -1) {
          return;
        }
        $rootScope.openedOptions = false;
        $rootScope.hideChooseText = true;
        return $timeout((function() {
          return $rootScope.$emit('open:city', options.start);
        }));
      }
    }
  ]);
  ...
  App.controller('OptionsController', [
    '$scope', '$rootScope', 'Rest', '$timeout', 'City', 'Options', function($scope, $rootScope, Rest, $timeout, City, Options) {
      var errors, resultToCity, resultToCityCoords, setStartOptions;
      setStartOptions = function() {
        $scope.startOptions = [Options.defaultOptions.start].concat(City.getAll().map(function(a) {
          return a.name;
        }));
        if ($scope.startOptions.indexOf($scope.options.start) === -1) {
      ...
      $scope.openCity = function(city) {
        if (City.has(city)) {
          $rootScope.$emit('open:city', city);
        }
        return $rootScope.openedOptions = false;
      };
      $scope.removeCity = function(name) {
        if (confirm("Are you sure want to delete " + name + "?")) {
          City.remove(name);
          $scope.cities = City.getAll();
          return setStartOptions();
        }
      };
